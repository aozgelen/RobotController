...
Localization::Localization(Map * map, 
  ...
  foundItem = false; 
  globalCameraAvailable = false;

  robotMutex.unlock();
}

void Localization::setBlobFinderProxy(PlayerClient* pc) { bfp = new BlobfinderProxy(pc, 0); }

void Localization::setPosition2dProxy(PlayerClient* pc) { p2d = new Position2dProxy(pc, 0); }

void Localization::setCameraProxy(PlayerClient* pc) { cam = new CameraProxy(pc, 0); }

Position Localization::getPosition() {
  Position pos(0,0,0) ;
    
  //if ( !isDestinationSet() ) {
  if( !globalCameraAvailable ) {
    robotMutex.lock(); 
...
double Localization::calcHeadingToDestination(double rx, double ry){
...
double Localization::getAngle(double x) {
  int xRes = 160;
  int halfXRes = 80;
...
/*double Localization::radiansToDegrees(double rad) {
  return rad * 180 / M_PI;
  }*/
 
/*
bool Localization::positionEqual(Position p1, Position p2) {
...
/* 
//BUGGY! Program chrashes with a SIGABRT when processing the blobs in this fashion. TODO: find out why
vector<Observation> Localization::findRoomMarkersFromBlobs(vector<player_blobfinder_blob>& topBlobs,
...
bool Localization::isOverlapping(observationBlob b1, observationBlob b2 ){
  bool overlap = false; 
  /* For cases:      
     1.
     b1
...
void Localization::displayObservationSummary(){
  if ( !obs.empty() ){
