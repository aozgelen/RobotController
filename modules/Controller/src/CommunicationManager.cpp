...
//CommunicationManager::CommunicationManager(PlayerClient& pc, Localization * i, string id, string type, Behavior* bp)
//  :mPlayerClient(pc), mIOService(), mSocket(mIOService), itl(i), mBehavior(bp), mNameID(id), mTypeID(type)
CommunicationManager::CommunicationManager(PlayerClient& pc, Localization * i, string id, string type)
  :mPlayerClient(pc), mIOService(), mSocket(mIOService), itl(i), mNameID(id), mTypeID(type)
{
  // setup interface to localization
  /*itl->setBlobFinderProxy(&pc); 
  itl->setPosition2dProxy(&pc); 
  itl->setCameraProxy(&pc);
  */
...
{
  // Prepend function signature to error messages.
  static const string signature = "CommunicationManager::init_provides()";
  ...
  try {
    ...
    if (COMMUNICATION_MANAGER_DEBUG) cerr << signature << " - failed to acquire capabilities from Player Server" << endl;
    dev.clear();
  }
	...
	if (!mPosition2D) {
	  ...
	  try {
		...
		mProvidesList.push_back(CAPS_POSITION2D);
	      ...
	      if (COMMUNICATION_MANAGER_DEBUG) cerr << signature << " - failed to allocate memory for Position2D proxy" << endl;
	    }
	  } catch (PlayerError) {
	    if (COMMUNICATION_MANAGER_DEBUG) cerr << signature << " - failed to subscribe to Position2D proxy" << endl;
	    delete pp;
	  }
	}
      } break;
...
void CommunicationManager::do_state_action_init()
{
  ...
  if (write(ss)) {
    ...
    if (COMMUNICATION_MANAGER_DEBUG) cerr << signature << " - timeout; next state: STATE_QUIT" << endl;
    do_state_change(STATE_QUIT);
  } else {
...
void CommunicationManager::do_state_action_ack()
{
  ...
  if (mStateTimer.elapsed() >= MAX_TIME_STATE) {
    if (COMMUNICATION_MANAGER_DEBUG) cerr << signature << " - timeout; next state: STATE_INIT" << endl;
    do_state_change(STATE_INIT);
  }
...
void CommunicationManager::do_state_action_idle()
{
  ...
  if (msg_waiting()) {
    ...
    if (COMMUNICATION_MANAGER_DEBUG) cerr << signature << " - max silence exceeded; next state: STATE_PING_SEND" << endl;
...
void CommunicationManager::do_state_action_pong_read()
{
  ...
  // Don't wait forever.
  if (mStateTimer.elapsed() >= MAX_TIME_STATE) {
    if (COMMUNICATION_MANAGER_DEBUG) cerr << signature << " - timeout; next state: STATE_PING_SEND" << endl;
    do_state_change(STATE_PING_SEND);
  }
...
void CommunicationManager::do_state_action_cmd_proc()
{
  ...
  if (read(ss) && (ss >> cmd)) {
	...
	mBehavior->Restart();
      ...
      */
      if (COMMUNICATION_MANAGER_DEBUG) cerr << signature << " - UNLOCK; next state: STATE_IDLE" << endl;
      do_state_change(STATE_IDLE);
    }
	...
	mBehavior->Stop();
      ...
      */
      if (COMMUNICATION_MANAGER_DEBUG) cerr << signature << " - LOCK; next state: STATE_IDLE" << endl;
      do_state_change(STATE_IDLE);
    }
    else if (cmd.find(CMD_MOVE) != string::npos) {
      // Save the command; it will be processed in the next state.
      mStringBuffer = ss.str();
      if (COMMUNICATION_MANAGER_DEBUG) cerr << signature << " - MOVE; next state: STATE_MOVING" << endl;
      do_state_change(STATE_MOVING);
    }
    else if (cmd.find(CMD_GOTO) != string::npos) {
      // Save the command; it will be processed in the next state.
      mStringBuffer = ss.str();
      if (COMMUNICATION_MANAGER_DEBUG) cerr << signature << " - GOTO; next state: STATE_GOTO" << endl;
      do_state_change(STATE_GOTO);
    }
    else if (cmd.find(CMD_CAMPOSE) != string::npos) {
      // Save the command; it will be processed in the next state.
      mStringBuffer = ss.str();
      if (COMMUNICATION_MANAGER_DEBUG) cerr << signature << " - CAMPOSE; next state: STATE_CAMPOSE" << endl;
      do_state_change(STATE_CAMPOSE);
    }
    else if (cmd.find(CMD_ASKPOSE) != string::npos) {
      if (COMMUNICATION_MANAGER_DEBUG) cerr << signature << " - ASKPOSE; next state: STATE_POSE" << endl;
      do_state_change(STATE_POSE);
    }
    else if (cmd.find(CMD_ASKPLAYER) != string::npos) {
      // Save the command; it will be used in the next state.
      mStringBuffer = ss.str();
      if (COMMUNICATION_MANAGER_DEBUG) cerr << signature << " - ASKPLAYER; next state: STATE_PLAYER" << endl;
...
void CommunicationManager::do_state_action_moving()
{
  // Prepend function signature to error messages.
  static const string signature = "CommunicationManager::do_state_action_moving()";
  ...
  try {
    ...
    if ( av != 0 ){ 
      ...
      cout << "accessing p2d directly from CommunicationManager.cpp" << endl;
...
void CommunicationManager::do_state_action_goto()
{
  // Prepend function signature to error messages.
  static const string signature = "CommunicationManager::do_state_action_goto()";
...
void CommunicationManager::do_state_action_campose(){
  // Prepend function signature to error messages.
  static const string signature = "CommunicationManager::do_state_action_campose()";
  ...
  double orient; 
  stringstream iss(mStringBuffer);
  if (!(iss >> command >> id >> xpos >> ypos >> orient ) 
      || (command.find(CMD_CAMPOSE) == string::npos) 
      || (mSessionID != id)) {
    stringstream oss;
    oss << CMD_ERROR << CMD_CAMPOSE << " failed: wrong arguments" << endl;
    write(oss);
    if (COMMUNICATION_MANAGER_DEBUG) cerr << signature << " - failure; next state: STATE_IDLE" << endl;
    do_state_change(STATE_IDLE);
    return;
  ...
  if ( !itl->isGlobalCameraAvailable() ){
    itl->setGlobalCamera(true); 
  }
  Position p(xpos, ypos, orient); 
  itl->setCurrentPosition(p);

  do_state_change(STATE_IDLE); 
}

void CommunicationManager::do_state_action_found(){
  // Prepend function signature to error messages.
  static const string signature = "CommunicationManager::do_state_action_found()";
...
void CommunicationManager::do_state_action_pose()
{
  // Prepend function signature to error messages.
  static const string signature = "CommunicationManager::do_state_action_pose()";
  ...
  // Not anymore it gets it directly from Localization
...
void CommunicationManager::do_state_action_player()
{
  // Prepend function signature to error messages.
  static const string signature = "CommunicationManager::do_state_action_player()";
